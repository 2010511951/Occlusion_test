...
'''
scc_map_img = Image.fromarray(occ_map)'''

occ_map_img = Image.fromarray(np.uint8(cm.hot(abs(occ_map))*255))

tf_img = Image.open("dataset/sob_lateral.bmp")
target_img = Image.open("dataset/Doc3_lateral.bmp")
...
blend_img.show()
blend_img.save("outcome/Sob-Doc3_blend.bmp")
